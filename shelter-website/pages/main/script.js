...
const createCardsSlider = () => {
    let id = [];
    cards.forEach(i => {
        id.push(i.dataset.id)
    })

   /* создаем каточку */
    cards.forEach((value) => {
        let image = value.querySelector('.pets__card-img');
        let name = value.querySelector('.pets__card-title');
        let random;

        //рандомное число
        while (true) {
            if (id.indexOf(String(random = getRandom(0, 7))) === -1) {
                break;
            }
        }

        image.src = pets[random].img;
        name.innerText = pets[random].name;
        value.dataset.id = pets[random].id;
...
/* анимация влево*/
const addSliderAnimationLeft = (e) => {
    document.querySelector('.pets__card').style.overflowX = 'hidden';
    cards.forEach(value => value.style.animation = 'sliderLeft 0.5s ease-in-out')
    e.target.disabled = true;
    setTimeout(() => {
        document.querySelector('.pets__card').style.overflowX = 'unset';
        e.target.disabled = false;
        cards.forEach(value => value.style.animation = '');
    }, 900);
}


sliderBtns[0].addEventListener('click', addSliderAnimationLeft);

sliderBtns.forEach((el) => el.addEventListener('click', () => {
    setTimeout(createCardsSlider, 400)
}));
...
/* анимация вправо*/
const addSliderAnimationRight = (e) => {
    document.querySelector('.pets__card').style.overflowX = 'hidden';
    cards.forEach(value => value.style.animation = 'sliderRights 0.5s ease-in-out')
    e.target.disabled = true;
    setTimeout(() => {
        document.querySelector('.pets__card').style.overflowX = 'unset';
        e.target.disabled = false;
...
sliderBtns[1].addEventListener('click', addSliderAnimationRight);
